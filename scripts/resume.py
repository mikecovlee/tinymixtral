...
_CPT_TRAINING_STATE_FIELDS = frozenset(
    {
        "cpt_state_version",
...
def validate_cpt_resume_state(state, model, optimizer):
    """Bind training_state.pt to the Native model's persistent CPT state."""
    if not isinstance(state, dict):
        raise RuntimeError("training_state.pt must contain a dictionary")
    unknown_cpt_fields = sorted(
        key for key in state if isinstance(key, str) and key.startswith("cpt_") and key not in _CPT_TRAINING_STATE_FIELDS
    )
    if unknown_cpt_fields:
        raise RuntimeError("training_state.pt contains unknown CPT fields: " + ", ".join(unknown_cpt_fields))
    ...
    if isinstance(saved_version, bool) or not isinstance(saved_version, int) or saved_version < 0:
        raise RuntimeError("training checkpoint cpt_state_version is invalid")
    model_version = model.get_cpt_state_version()
    if saved_version != model_version:
        raise RuntimeError("training_state.pt cpt_state_version disagrees with the model checkpoint")
    presence = state["cpt_optimizer_state_presence"]
    if not isinstance(presence, (list, tuple)) or any(not isinstance(name, str) for name in presence):
        raise RuntimeError("training checkpoint cpt_optimizer_state_presence is invalid")
    ...
    validate_serialized_cpt_optimizer_state(
        ...
        expected_parameter_bindings=state["cpt_optimizer_parameter_bindings"],
        expected_state_dtype=state["cpt_optimizer_state_dtype"],
    )
    return saved_version


def restore_cpt_optimizer_state(
    state,
    model,
    optimizer,
    *,
    ...
    """Validate and restore grouped or legacy single-group CPT AdamW state."""
    expected_optimizer_type = BF16AdamW if bf16_states else torch.optim.AdamW
    if type(optimizer) is not expected_optimizer_type:
        raise TypeError("live optimizer type disagrees with the requested BF16 state policy")
    serialized_optimizer = state.get("opt") if isinstance(state, dict) else None
    serialized_groups = serialized_optimizer.get("param_groups") if isinstance(serialized_optimizer, dict) else None
    legacy_single_group = (
        isinstance(serialized_groups, list) and len(serialized_groups) == 1 and len(optimizer.param_groups) != 1
    )
    restored_optimizer = optimizer
    if legacy_single_group:
        restored_optimizer = expected_optimizer_type(
            model.parameters(),
            lr=lr,
    ...
    validate_cpt_resume_state(state, model, restored_optimizer)
    ...
    return restored_optimizer, legacy_single_group
...
def main():
    ...
    p.add_argument("--max-tokens", type=int, default=None, help="覆盖 checkpoint 中的训练目标（post-training 用）")
    p.add_argument("--output-dir", default=None, help="输出目录（默认同 checkpoint-dir，post-training 建议指定新目录）")
    p.add_argument("--save-every-min", type=int, default=120)
    p.add_argument("--keep-last-checkpoints", type=int, default=5)
    p.add_argument("--log-every", type=int, default=100)
    p.add_argument(
        "--schedule", default="cosine", choices=["cosine", "wsd"], help="LR schedule: cosine or wsd (Warmup-Stable-Decay)"
    )
    p.add_argument("--bf16-optim", action="store_true", help="优化器状态使用 bf16 存储 (节省约 50%% 优化器显存)")
    p.add_argument("--eval-on-save", action="store_true", help="每次保存后同步执行 CPU GLUE 评测")
    args = p.parse_args()
    ...
    ckpts = sorted(
        [
            ...
            if d.is_dir() and d.name.startswith("step_") and all((d / name).is_file() for name in required_files)
        ]
    )
    if not ckpts:
    ...
    if args.seq_len > model.config.max_position_embeddings:
        p.error(f"seq-len {args.seq_len} exceeds model limit " f"{model.config.max_position_embeddings}")
    ...
    if state_path.exists():
        ...
        opt, legacy_single_group = restore_cpt_optimizer_state(
            state,
            model,
            opt,
            lr=args.lr,
        ...
        if saved_bs is not None and (saved_bs != bs or saved_seq != seq):
            p.error(f"checkpoint uses batch-size={saved_bs}, seq-len={saved_seq}; " f"got batch-size={bs}, seq-len={seq}")
    else:
        state = {}
    ...
    if is_posttrain:
        ...
        print(
            f"Post-training target: {total_steps:,} steps / ~{target_tokens / 1e9:.2f}B tokens "
            f"lr={args.lr:.1e} warmup={warmup}",
            flush=True,
        )
    else:
    ...
    if is_posttrain:
        ...
        total_corpus_tokens = sum(len(torch.load(f, weights_only=True, mmap=True)) for f in files)
    ...
    step, total_tok, fi, ptr, elapsed = training_loop(
        ...
        save_every_min=args.save_every_min,
