...
def main():
    ...
    target.add_argument("--max-tokens", type=int, default=None, help="训练 token 目标（默认 4,000,000,000）")
    target.add_argument("--max-steps", type=int, default=None, help="训练 step 目标")
    ...
    print(
        f"Model: {cfg.hidden_size}d/{cfg.num_hidden_layers}L/{cfg.num_local_experts}E "
    ...
    print(f"Training target: {total_steps:,} steps / {target_tokens / 1e9:.3f}B tokens " f"bs={bs} seq={seq}", flush=True)
