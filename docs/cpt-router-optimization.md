...
2. **修复对称性破缺**的候选:更大的 `energy_init_scale`、更低专家温度 `τ_e`、显式分化压力(B 行多样性目标/原型-专家指派)、或给 CPT 补充类似 jitter 的探索项。
3. 修复后重跑端到端,再判断 CPT 框架在此规模的价值。

**边界**:第二级为小规模(hidden 256、100M token)、每变体单次运行;路由结构差异(B 行余弦、pair_div)幅度大且三变体一致,不太可能是噪声,但 loss 差异可能部分在噪声范围内。
