...
class SparseMoE(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.training:
            ...
            out = out + keepalive.to(out.dtype)
...
class TinyMixtralForCausalLM(PreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config):
        ...
        if config.tie_word_embeddings:
            ...
            self._tied_weights_keys = {"lm_head.weight": "embed_tokens.weight"}
    ...
    def forward(self, input_ids, attention_mask=None, labels=None, return_dict=True, past_key_values=None, use_cache=False, **kwargs):
        ...
        past_len = past_key_values[0][0].shape[2] if has_cache else 0
        ...
        for i, layer in enumerate(self.layers):
            layer_cache = past_key_values[i] if has_cache else None
